...
#     """
#     def __init__(self, reduction='mean', margin=1.0, T=1.0, epsilon=1e-6):
#         super(LatentSimilarity, self).__init__()
#         self.margin = margin  # margin for controlling minimum distance
...
#         # Reshape to (b*h*w, c) for pairwise comparison
...
#         distance = torch.norm(x1 - x2, p=2, dim=1)  # Euclidean distance
        
#         # Select only the changed regions (label_change = 1)
#         mask_change = label_change.bool().reshape(b * h * w)  # mask for changed regions
#         distance_change = distance[mask_change]  # distances for changed regions
        
#         # Apply margin: penalize small distances
#         loss = -torch.log(distance_change + self.epsilon) 
#         # loss = -torch.log(distance_change + self.epsilon) + F.relu(self.margin - distance_change)
...
class LatentSimilarity(nn.Module):
    ...
    def forward(self, x1, x2, label_change):
        ...
        # Compute L2 distance between corresponding features
        distance = torch.norm(x2 - x1, p=2, dim=1)  # Euclidean distance
        distance = distance / (torch.norm(x1, p=2, dim=1) + self.epsilon)
        ...
        # Apply margin: penalize small distances
        # loss = -torch.log(distance_change + self.epsilon)
        loss = 0.5 * (torch.tanh(-torch.log(distance_change + self.epsilon)) + 1)
